...
- (id)copyWithZone:(NSZone *)zone {
    TBPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
@interface TBPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) TBPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation TBPhotoGroupCell
...
- (void)setItem:(TBPhotoGroupItem *)item {
...
@interface TBPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation TBPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((TBPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    TBPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    TBPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    TBPhotoGroupCell *cell = [self cellForPage:currentPage];
    TBPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(TBPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            TBPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                TBPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (TBPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (TBPhotoGroupCell *)dequeueReusableCell {
    TBPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (TBPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (TBPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    if (!_isPresented)
        return;
    TBPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
